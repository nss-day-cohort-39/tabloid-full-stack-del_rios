...
const PostDetails = () => {
  ...
  const userTypeId = JSON.parse(sessionStorage.getItem("userProfile"))
    .userTypeId;
  const { id } = useParams();
    ...
    if (post.userProfileId === userProfileId) {
      return (
        <ListGroupItem className="buttonContainer">
          ...
          <Button onClick={toggleToast} color="danger">
            Delete Post
          </Button>
          <Button onClick={toggleComments} color="primary">
        ...
        <div className="col-sm-12 col-lg-6">
          <ListGroup>
            ...
            {post.publishDateTime == null ? (
              <ListGroupItem>
                <strong>Posted: </strong>No publication date.
              </ListGroupItem>
            ) : (
              <ListGroupItem>
            ...
            <ListGroupItem>
              ...
              <Link to={`/user/${post.userProfileId}`}>
            ...
            <ListGroupItem>
              <div className="postTags">
                ...
                {post.postTags.map((pt) => (
                  <TagsOnPost key={pt.id} postTag={pt} />
                ))}
              </div>
            </ListGroupItem>
            ...
            {post.userProfileId === userProfileId ? (
              <ListGroupItem>
                <Link to={`/AddTagForm/post/${post.id}`}>
                  <h6>Manage Tags</h6>
                </Link>
              </ListGroupItem>
            ) : (
            ...
            {post.isApproved === false ? (
              <p>
                This post has not yet been approved by an admin and is only
                visible to you.
              </p>
            ) : (
              ""
          ...
          <div className="p-3 my-2 rounded">
            <Toast isOpen={showToast}>
              <ToastHeader className="bg-danger">Warning</ToastHeader>
              <ToastBody>
                  ...
                  <Form onSubmit={updatePost}>
                    ...
                    <FormGroup>
                      ...
                      <select
                        ...
                        defaultValue={
                          categories.find(
                            (c) => c.id === post.category.id && c.active
                          )
                            ? post.category.id
                            : ""
                        }
